...
if (PHASE >= 16) {
  ...
  const P16 = [
    ...
    ["reports are append-only: no update policy",                    () => probe16(U["org.owner@bes.test"], `select public.create_credit_report('${lakesideOrg}', null, '${T.lakeside_client}', null, array['EQ'], current_date, 'manual_upload', null, 'probe-1', ${ITEMS}, null); update public.credit_reports set parser_version='x' where fulfillment_client_id='${T.lakeside_client}'; select count(*)::int as rows from public.credit_reports where parser_version='x'`), canSeeLakesideClient(U["org.owner@bes.test"]) ? 0 : "ERR 42501"],
