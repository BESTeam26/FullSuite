-- 0336 — a direct message says WHO it is with, by id.
--
-- 0335 gave the list the other person's NAME, which is right for a label and
-- wrong for identity. The portal has to pair "Rowell, your processor" in the
-- account team with the direct conversation that already exists with him, and
-- doing that by matching display names breaks the moment two people share one
-- — quietly, by showing somebody the wrong conversation.
--
-- Rule 4: never infer a relationship from a display name. So the list returns
-- the id as well, and the interface pairs on that.
...
returns table (
  ...
  service_name text, kind text, name text, display_name text, direct_user_id uuid,
  purpose text, open_to_scope boolean, archived_at timestamptz, shared_with_bes boolean,
...
language sql stable security invoker set search_path = public as $function$
  select
    ...
    case when c.kind = 'direct' then (
      select dm.user_id from public.channel_members dm
       where dm.channel_id = c.id and dm.user_id <> auth.uid()
       limit 1
    ) end,
