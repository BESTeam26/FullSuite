-- =============================================================================
-- One client, imported, in one transaction.
--
-- The Edge Function fetches and parses; this decides and writes. Everything
-- that must be true together is true together: if the secrets fail, the client
-- is not left half-imported with an empty vault.
--
-- ── WHY THE WRITE LIVES HERE AND NOT IN THE FUNCTION ───────────────────────
--
-- Authorization, idempotency and audit are all database concerns already. A
-- TypeScript importer that opened its own connection would be a second place
-- where those rules are decided, and the second place is always the one that
-- drifts. The Edge Function holds the ClickUp token and the parser; it holds
-- no rules.
--
-- ── IDEMPOTENCY ─────────────────────────────────────────────────────────────
--
-- `client_match_for_import` decides, task id first. A second run therefore
-- ENRICHES the same records — Dee's requirement that the six already imported
-- stay canonical and are not wiped and recreated. Notes and secrets are keyed
-- on their ClickUp ids too, so nothing stacks.
-- =============================================================================
