-- Found by the matrix: `text[] || 'literal'` makes Postgres parse the literal as an
-- array (22P02), so every identifier match crashed the provisioning trigger and the
-- trial-abuse check never blocked or flagged anything. array_append is unambiguous.
