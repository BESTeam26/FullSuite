-- Invitation / Activation V2 — the lifecycle Dee locked on 2026-09-20:
--
...
--
-- She accepted acceptance-time creation on six conditions. Four already held;
-- these are the two that did not, plus the proof for all six lives in
-- supabase/scripts/invitation-lifecycle-probe.mjs.
--
--   2. "Agent ID generation is idempotent and unique" — the formula cannot
--      collide with itself, but it CAN collide with a code the owner set by
--      hand, and the unique index would then fail the whole acceptance. The
--      generator now steps past a taken code instead.
--   5. "superseded invitations cannot create stale records" — re-inviting an
--      address rotates the token (000100/001200); it also has to drop the
--      staging the old invitation carried, or a person re-invited into a
--      different department would arrive with the first one's seat.

/** The first free code at or after this one. Same input, same output. */
create or replace function public.employee_code_free(p_agency uuid, p_code text) returns text
language plpgsql stable set search_path = public as $function$
declare v_base text; v_seq int; v_try text;
begin
  if p_code is null then return null; end if;
  v_base := left(p_code, length(p_code) - 3);
  v_seq := coalesce(nullif(right(p_code, 3), '')::int, 1);
  for i in 0..999 loop
    v_try := v_base || lpad((v_seq + i)::text, 3, '0');
    if not exists (select 1 from public.agency_memberships m where m.agency_id = p_agency and m.employee_code = v_try) then
      return v_try;
    end if;
  end loop;
  return p_code;
end $function$;
revoke all on function public.employee_code_free(uuid, text) from public, anon, authenticated;
...
begin
  if new.employee_code is null then
    ...
    /* Idempotent: a membership that already has a code keeps it, so a second
       acceptance never re-numbers anybody. Unique: a code somebody already
       holds is stepped past rather than colliding. */
    new.employee_code := public.employee_code_free(
      new.agency_id,
      public.employee_code_for(new.agency_id, new.id, new.user_id, coalesce(new.hired_on::timestamptz, new.created_at), coalesce(new.is_owner, false)));
  end if;
  return new;
end $function$;
...
begin
  ...
  if v_id is not null then
    update public.invitations
     ...
     where id = v_id;
    /* A superseded invitation must not carry the OLD placement forward: the
       caller re-stages whatever is true now (Dee, 2026-09-20 — "superseded
       invitations cannot create stale records"). */
    delete from public.invitation_onboarding where invitation_id = v_id;
  else
