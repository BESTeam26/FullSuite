...
| A2 | **A Resend API key with *sending* access, and a verified sending domain** (Resend → Domains). The mail functions call `api.resend.com/emails`. | `npx supabase secrets set MAIL_PROVIDER_API_KEY=re_… MAIL_FROM="BES <no-reply@yourdomain>"` | The branded "activate your account" email on an invitation, and the one-time welcome when a new organization's workspace is ready. Without it both are still *recorded* — the link can be copied — and the screen says the email did not go. |
| A2b | **Resend SMTP credentials into Supabase** (Authentication → Emails → SMTP: host `smtp.resend.com`, port 587, username the literal word `resend`, password a Resend API key) | Supabase dashboard | **Sign-up confirmation and password-reset emails.** Supabase's built-in sender allows only a few an hour, which is not enough to test sign-up properly. |
