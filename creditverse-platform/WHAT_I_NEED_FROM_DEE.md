...
| **B8** | **The 140 BES CRM build tasks** — the tracker list itself | You referred to it as designed; it is **not in this repository** and I do not have it, so I have not guessed at it. Send the list (paste, sheet, ClickUp list id, anything) and it goes into the repo verbatim first. The target shape is already settled and needs no design work: each task becomes a canonical `work_items` row with `division = 'bes_crm'` and the customer in `subject_organization_id` — the same shape a BES CRM project already uses, which is why it already flows to Production, EOD and the customer's read-only view. |
