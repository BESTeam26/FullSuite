...
14. **Positions as first-class records** (§5–6, §31 of the 2026-09-07 reconciliation) — `BES AGENCY HQ → DIVISION → DEPARTMENT → TEAM → POSITION → PERSON`. Designed and written as a migration on 2026-09-08, then **parked unapplied** when Dee's Communication release took priority; the draft is not in the repository. The design: a `positions` table (a seat that exists whether or not anybody is in it, with `reports_to_id` drawn seat-to-seat so a promotion does not redraw the chart, and a headcount), plus dated `position_assignments` with `kind` in (`holder`, `acting`) that are ENDED and never deleted — which is the whole mechanism by which "who held this in March" keeps its answer when somebody moves in April. State (`filled` / `covered` / `vacant`) is derived, never stored. Rebuild from this description; nothing about it is read by any authorization helper and nothing about it may ever be
15. **Communication follow-ons** — an @-mention autocomplete in the composer (the notifier is correct as of 0206, but the composer emits plain text so no mention marks exist to notify on); realtime delivery of other people's messages; message editing UI; Google/Zoom OAuth and the Edge Function behind the meeting model
