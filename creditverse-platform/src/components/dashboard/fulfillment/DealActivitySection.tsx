...
export function DealActivitySection({
  ...
  // Map FundingActivityEntry to ActivityEntry-like structure
  const mappedEntries: ActivityEntry[] = useMemo(() => {
  ...
  const { pinned, chronological } = useMemo(() => {
    const pinnedList = mappedEntries.filter((a) => a.pinned);
    const rest = mappedEntries.filter((a) => !a.pinned);
    rest.sort(
      (a, b) =>
        ...
        >
          {activity.length === 0 ? (
            <p className="text-xs italic text-muted-foreground">
              No activity yet. Status changes, comments, submissions, lender
              responses, stipulations, offers, and completed work are logged
              here.
            </p>
          ) : (
            <>
                ...
                <div className="space-y-2 rounded-lg border border-dashed border-primary/40 bg-primary/5 p-2">
                  ...
                  {pinned.map((a) => (
                    <ActivityCard
                      ...
                      onPin={() => dealStore.togglePin(dealId, a.id)}
                      onMark={(m) =>
                        dealStore.setMark(
                          dealId,
                          a.id,
                          m as "urgent" | "resolved" | "flagged" | "info",
                        )
                      }
