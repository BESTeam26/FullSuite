/**
 * Fulfillment Clients panel — Main Client List for the selected Partner.
 *
...
export function FulfillmentClientsPanel({
  ...
  const activeCount = filtered.filter(
    (c) => !["Completed", "Archived", "Graduated"].includes(c.status),
  ).length;
  ...
  const hideModeCol =
    !!partner &&
    (partner.group === "managed" || partner.group === "outsourcing");
  ...
  return (
    <div className="space-y-4">
      ...
      {selectedScope !== "all" &&
        seedOutsourcingGroups.some((g) => g.id === selectedScope) && (
          <div className="mt-6">
