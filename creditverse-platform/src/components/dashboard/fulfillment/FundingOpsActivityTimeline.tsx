/**
 * FundingOps Activity & Rich Comment Timeline.
 *
 * Mirrors ClientWorkActivityTimeline (CreditOps) but reads/writes the
 * FundingOps client store. ClickUp / Slack-style:
 ...
 *   - Inline attachment previews; paste images (Ctrl+V) or attach files.
 */
...
export function FundingOpsActivityTimeline({ clientId }: Props) {
  ...
  // FundingActivityEntry is structurally identical to ActivityEntry.
  const mapped = activity as unknown as ActivityEntry[];
  ...
  const { pinned, chronological } = useMemo(() => {
    const pinnedList = mapped.filter((a) => a.pinned);
    const rest = mapped.filter((a) => !a.pinned);
    rest.sort(
      (a, b) =>
  ...
  // Clipboard paste inside textarea
  useEffect(() => {
        ...
        >
          {activity.length === 0 ? (
            <p className="text-xs italic text-muted-foreground">
              No activity yet. Status changes, comments, and updates are logged
              here.
            </p>
          ) : (
            <>
