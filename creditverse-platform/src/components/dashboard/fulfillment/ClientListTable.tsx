/**
 * ClientListTable — the list view of the Main Client List.
...
export function ClientListTable({
  ...
  const renderCell = (client: FulfillmentClient, colId: ColId): ReactNode => {
    switch (colId) {
      case "client":
        return (
      ...
      case "round":
        return (
          <span className="font-semibold text-foreground">{client.round}</span>
        );
      case "status":
        ...
        return (
          <button onClick={() => setEditingStatusId(client.id)}>
            <FulfillmentStatusPill status={client.status} />
          </button>
        );
      case "agent":
