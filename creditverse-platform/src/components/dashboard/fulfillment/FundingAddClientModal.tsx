/**
 * Funding Add Client Modal — creates a manual funding client record.
 *
...
export function FundingAddClientModal({
  ...
  const [status, setStatus] = useState<FundingClient["status"]>("Onboarding");
  const [assignee, setAssignee] = useState("Unassigned");
  const [requested, setRequested] = useState("");
  const [errors, setErrors] = useState<Record<string, string>>({});
  /** Cross-partner matches awaiting explicit confirmation. */
  const [pendingConfirm, setPendingConfirm] = useState<FundingClient[] | null>(
    null,
  );

  if (!open) return null;

  const isNative = partner?.mode === "native_fundingops";

  const validate = () => {
    const e: Record<string, string> = {};
    if (!name.trim()) e.name = "Name is required";
    if (!email.trim()) e.email = "Email is required";
    else if (!FUNDING_EMAIL_RE.test(email)) e.email = "Invalid email format";
    if (phone && !FUNDING_PHONE_RE.test(phone))
      ...
      <div className="w-full max-w-lg rounded-2xl border border-border bg-card shadow-xl">
        <div className="flex items-center justify-between border-b border-border px-5 py-3.5">
          <h3 className="flex items-center gap-2 text-sm font-bold text-foreground">
            <UserPlus className="h-4 w-4 text-primary" /> Add Funding Client
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="space-y-4 p-5">
          {isNative && (
            <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-3 text-xs text-emerald-700">
              This Partner uses the native BES FundingOps SaaS connection. New
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <select
                value={status}
                onChange={(e) =>
                  setStatus(e.target.value as FundingClient["status"])
                }
              ...
              >
                ...
                <option>Lender Matching</option>
                <option>Submitted</option>
                <option>Stipulations</option>
