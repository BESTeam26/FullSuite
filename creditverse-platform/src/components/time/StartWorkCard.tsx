 ...
 * would need a picker for division and partner anyway, so the picker is right
 * here and the box stays a filter over what you actually do.
 *
 * The picker is ALWAYS VISIBLE (Dee, 2026-09-20). It used to sit behind a
 * "Choose manually" toggle, which meant somebody clocking in for the first
 * time — with no recent work to quick-start from — had to guess that a link
 * was the way in. On the one screen everybody uses every morning, a hidden
 * primary action is friction, not tidiness.
 *
    ...
    <div className="rounded-2xl border border-border bg-card p-5">
      ...
      {matches.length === 0 ? (
        <p className="mt-2 text-xs text-muted-foreground">
          {query.trim()
            ? "Nothing you have tracked matches that. Start one below."
            : "Nothing tracked yet — start your first timer below."}
        </p>
      ) : (
        ...
        <div className="mt-2 space-y-2">
          ...
          <label className="block text-[11px] font-medium text-muted-foreground">
            ...
            <input value={note} onChange={(e) => setNote(e.target.value)}
              ...
              placeholder="Optional — helps your End of Day write itself"
              className="mt-0.5 w-full rounded-lg border border-border bg-background px-3 py-2 text-xs text-foreground placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" />
          </label>
