...
export const DiyConsumerShell = ({ children }: { children?: ReactNode }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-charcoal text-white">
      <header className="sticky top-0 z-20 border-b border-white/10 bg-charcoal-deep/80 backdrop-blur">
        <div className="flex h-16 items-center justify-between px-4 md:px-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="hidden text-xs text-slate-400 sm:block">
              Signed in as{" "}
              <span className="font-medium text-white">Maria T.</span>
            </span>
            <button className="flex items-center gap-1.5 rounded-lg border border-white/10 px-3 py-1.5 text-xs font-medium text-slate-300 hover:bg-white/10">
      ...
      <div className="flex">
        <aside className="sticky top-16 hidden h-[calc(100vh-4rem)] w-60 shrink-0 border-r border-white/10 bg-charcoal-deep/40 p-3 md:block">
        ...
        <div className="flex w-full overflow-x-auto border-b border-white/10 px-2 py-2 md:hidden">
          {nav.map((n) => (
            <button
              key={n.key}
              onClick={() => setView(n.key)}
