...
describe("helpFor", () => {
  ...
  it("covers the main screens with user-facing wording", () => {
    expect(HELP_ROUTE_COUNT).toBeGreaterThanOrEqual(27);
    for (const path of ["/app/my-work", "/app/reporting", "/app/settings", "/app/dispute-dashboard", "/app/funding-dashboard", "/app/people", "/app/files", "/app/tools"]) {
