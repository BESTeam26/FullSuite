...
export async function fetchCutoffs(): Promise<PayrollCutoff[]> {
  ...
  const { data, error } = await sb
    ...
    .select("id, period_start, period_end, status, released_at, expense_id")
...
/** The caller's own payslips across cutoffs, newest first. */
export async function fetchMyPayslips(userId: string): Promise<(Payslip & { periodStart: string; periodEnd: string; released: boolean })[]> {
  const sb = requireSupabase();
  const { data, error } = await sb
    .from("payslips")
    .select(PAYSLIP_SELECT + ", payroll_cutoffs(period_start, period_end, status)")
    .eq("user_id", userId)
    .order("created_at", { ascending: false }).limit(24);
  if (error) throw error;
  return (data ?? []).map((row) => {
    const r = row as unknown as Record<string, unknown>;
    const c = r.payroll_cutoffs as { period_start: string; period_end: string; status: string };
    return { ...mapPayslip(r), periodStart: c.period_start, periodEnd: c.period_end, released: c.status === "released" };
  });
}
