              ...
              <li key={f.id} className="flex items-center justify-between gap-2 py-2">
                <span className="min-w-0">
                  ...
                  <span className="block text-[11px] text-muted-foreground">
                    {f.sharedAt ? `Shared ${formatDate(f.sharedAt.slice(0, 10))}` : ""}
                ...
                <button type="button" onClick={() => void open(f.id, f.path)}
                  className={cn("flex shrink-0 items-center gap-1 rounded-lg border border-border px-2.5 py-1.5 text-xs font-medium text-foreground",
